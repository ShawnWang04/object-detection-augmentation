...
#-----------------------------------------------------------------------------------#
Origin_VOCdevkit_path   = "D:\\test\\data"
Out_VOCdevkit_path      = "D:\\test\\wenjia1"
#-----------------------------------------------------------------------------------#
#   Out_Num                 利用mixup生成多少组图片
#   input_shape             生成的图片大小
#-----------------------------------------------------------------------------------#
Out_Num                 = 700
input_shape             = [3000 , 5472]
...
if __name__ == "__main__":
    Origin_JPEGImages_path  = os.path.join(Origin_VOCdevkit_path, "image")
    Origin_Annotations_path = os.path.join(Origin_VOCdevkit_path, "lable")
    
    Out_JPEGImages_path  = os.path.join(Out_VOCdevkit_path, "JPEGImages")
    Out_Annotations_path = os.path.join(Out_VOCdevkit_path, "Annotations")
    ...
    for index in range(Out_Num):
        ...
        #   获取两个图像与标签
        #------------------------------#
        sample_xmls = sample(xml_names, 3)
